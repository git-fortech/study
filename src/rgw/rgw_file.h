...
public:
    ...
    RGWFileHandle* get_parent()
    {
        return parent;
    }

    uint32_t get_owner_uid() const
    {
        return state.owner_uid;
    }
    uint32_t get_owner_gid() const
    {
        return state.owner_gid;
    }
    ...
    bool is_open() const
    {
        return flags & FLAG_OPEN;
    }
    bool is_root() const
    {
        return flags & FLAG_ROOT;
    }
    bool is_bucket() const
    {
        return flags & FLAG_BUCKET;
    }
    ...
    bool is_file() const
    {
        return (fh.fh_type == RGW_FS_TYPE_FILE);
    }
    bool is_dir() const
    {
        return (fh.fh_type == RGW_FS_TYPE_DIRECTORY);
    }
    bool creating() const
    {
        return flags & FLAG_CREATING;
    }
    ...
    void encode(buffer::list& bl) const
    {
        ...
        for (const auto& t :
    {
        state.ctime, state.mtime, state.atime
    })
        {
...
public:
    ...
    std::map<string, bufferlist>& get_attrs()
    {
        return attrs;
    }
