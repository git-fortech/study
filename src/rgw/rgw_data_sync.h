...
public:
    ...
    rgw_data_sync_status& get_sync_status()
    {
        return sync_status;
    }
    ...
    {
        return source_log.init_sync_status(num_shards);
    }
    ...
    int run()
    {
        return source_log.run_sync(num_shards, sync_status);
    }

    void wakeup(int shard_id, set<string>& keys)
    {
        return source_log.wakeup(shard_id, keys);
    }
    void stop()
    {
        source_log.finish();
...
public:
    ...
    map<int, rgw_bucket_shard_sync_info>& get_sync_status()
    {
        return sync_status;
    }
    int init_sync_status();
