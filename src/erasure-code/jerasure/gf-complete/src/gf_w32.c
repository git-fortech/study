...
gf_w32_split_4_32_lazy_sse_altmap_multiply_region(gf_t *gf, void *src, void *dest, uint32_t val, int bytes, int xor)
{
    ...
    if (xor)
    {
        while (d32 != top)
        {
            ...
            v0 = _mm_load_si128((__m128i *) s32);
            s32 += 4;
            v1 = _mm_load_si128((__m128i *) s32);
            s32 += 4;
            v2 = _mm_load_si128((__m128i *) s32);
            s32 += 4;
            v3 = _mm_load_si128((__m128i *) s32);
            s32 += 4;
...
int gf_w32_init(gf_t *gf)
{
    ...
    {
    case GF_MULT_CARRY_FREE:
        if (gf_w32_cfm_init(gf) == 0) return 0;
        break;
    case GF_MULT_CARRY_FREE_GK:
        if (gf_w32_cfmgk_init(gf) == 0) return 0;
        break;
    case GF_MULT_SHIFT:
        if (gf_w32_shift_init(gf) == 0) return 0;
        break;
    case GF_MULT_COMPOSITE:
    ...
    case GF_MULT_DEFAULT:
    case GF_MULT_SPLIT_TABLE:
        if (gf_w32_split_init(gf) == 0) return 0;
        break;
    case GF_MULT_GROUP:
        if (gf_w32_group_init(gf) == 0) return 0;
        break;
    case GF_MULT_BYTWO_p:
    case GF_MULT_BYTWO_b:
        if (gf_w32_bytwo_init(gf) == 0) return 0;
        break;
    default:
        return 0;
    }
