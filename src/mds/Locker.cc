...
bool Locker::simple_sync(SimpleLock *lock, bool *need_issue)
{
    ...
    if (old_state != LOCK_TSYN)
    {
        ...
        case LOCK_LOCK:
            ...
            break;
        case LOCK_XSYN:
            lock->set_state(LOCK_XSYN_SYNC);
            break;
        case LOCK_EXCL:
            lock->set_state(LOCK_EXCL_SYNC);
            break;
        default:
            assert(0);
        }
...
void Locker::simple_excl(SimpleLock *lock, bool *need_issue)
{
    ...
    {
    case LOCK_LOCK:
        lock->set_state(LOCK_LOCK_EXCL);
        break;
    case LOCK_SYNC:
        lock->set_state(LOCK_SYNC_EXCL);
        break;
    case LOCK_XSYN:
        lock->set_state(LOCK_XSYN_EXCL);
        break;
    default:
        assert(0);
    }
...
void Locker::simple_xlock(SimpleLock *lock)
{
    ...
    case LOCK_LOCK:
    case LOCK_XLOCKDONE:
        lock->set_state(LOCK_LOCK_XLOCK);
        break;
    default:
        assert(0);
    }
...
void Locker::scatter_mix(ScatterLock *lock, bool *need_issue)
{
    ...
    else
    {
        ...
        // fall-thru
        case LOCK_EXCL:
            lock->set_state(LOCK_EXCL_MIX);
            break;
        case LOCK_TSYN:
            lock->set_state(LOCK_TSYN_MIX);
            break;
        default:
            assert(0);
        }
