...
public:
    ...
    const char *generic_pin_name(int p) const
    {
        ...
        case PIN_WAITER:
            return "waiter";
        case PIN_DIRTYSCATTERED:
            return "dirtyscattered";
        case PIN_AUTHPIN:
            return "authpin";
        case PIN_PTRWAITER:
            return "ptrwaiter";
        case PIN_TEMPEXPORTING:
            return "tempexporting";
        case PIN_CLIENTLEASE:
            return "clientlease";
        default:
            assert(0);
            return 0;
...
public:
    ...
    unsigned state_test(unsigned mask) const
    {
        return (state & mask);
    }
    ...
    bool is_auth() const
    {
        return state_test(STATE_AUTH);
    }
    bool is_dirty() const
    {
        return state_test(STATE_DIRTY);
    }
    bool is_clean() const
    {
        return !is_dirty();
    ...
    {
        return state_test(STATE_REJOINING);
    }
...
public:
    bool is_auth_pinned() const
    {
        return auth_pins || nested_auth_pins;
    }
    int get_num_auth_pins() const
    {
        return auth_pins;
    }
    int get_num_nested_auth_pins() const
    {
...
public:
    ...
    {
        return !replica_map.empty();
    }
    bool is_replica(mds_rank_t mds) const
    {
        return replica_map.count(mds);
    }
    ...
    compact_map<mds_rank_t,unsigned>::iterator replicas_begin()
    {
        return replica_map.begin();
    }
    compact_map<mds_rank_t,unsigned>::iterator replicas_end()
    {
        return replica_map.end();
    }
    const compact_map<mds_rank_t,unsigned>& get_replicas() const
    {
        return replica_map;
    }
    ...
    unsigned get_replica_nonce() const
    {
        return replica_nonce;
    }
    void set_replica_nonce(unsigned n)
    {
        replica_nonce = n;
    }
...
//				   << dendl;
    ...
    virtual void encode_lock_state(int type, bufferlist& bl)
    {
        assert(0);
    }
    virtual void decode_lock_state(int type, bufferlist& bl)
    {
        assert(0);
    }
    virtual void finish_lock_waiters(int type, uint64_t mask, int r=0)
    {
        assert(0);
    }
    virtual void add_lock_waiter(int type, uint64_t mask, MDSInternalContextBase *c)
    {
        assert(0);
    }
