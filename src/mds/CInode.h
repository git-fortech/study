...
public:
    ...
    const char *pin_name(int p) const
    {
        ...
        case PIN_OPENINGDIR:
            return "openingdir";
        case PIN_REMOTEPARENT:
            return "remoteparent";
        case PIN_BATCHOPENJOURNAL:
        ...
        case PIN_FROZEN:
            return "frozen";
        case PIN_IMPORTINGCAPS:
            return "importingcaps";
        case PIN_EXPORTINGCAPS:
            return "exportingcaps";
        case PIN_PASTSNAPPARENT:
            return "pastsnapparent";
        case PIN_OPENINGSNAPPARENTS:
            return "openingsnapparents";
        case PIN_TRUNCATING:
            return "truncating";
        case PIN_STRAY:
            return "stray";
        case PIN_NEEDSNAPFLUSH:
            return "needsnapflush";
        case PIN_DIRTYRSTAT:
            return "dirtyrstat";
        case PIN_DIRTYPARENT:
            return "dirtyparent";
        case PIN_DIRWAITER:
            return "dirwaiter";
        case PIN_SCRUBQUEUE:
...
public:
    ...
    bool is_root() const
    {
        return inode.ino == MDS_INO_ROOT;
    }
    bool is_stray() const
    {
        return MDS_INO_IS_STRAY(inode.ino);
    }
    mds_rank_t get_stray_owner() const
    {
        return (mds_rank_t)MDS_INO_STRAY_OWNER(inode.ino);
    }
    bool is_mdsdir() const
    {
        return MDS_INO_IS_MDSDIR(inode.ino);
    }
    bool is_base() const
    {
        return is_root() || is_mdsdir();
    }
    bool is_system() const
    {
        return inode.ino < MDS_INO_SYSTEM_BASE;
    }

    bool is_head() const
    {
        return last == CEPH_NOSNAP;
    }
    ...
    {
        return vinodeno_t(inode.ino, last);
    }
    int d_type() const
    {
        return IFTODT(inode.mode);
    }
    ...
    CDentry* get_parent_dn()
    {
        return parent;
    }
    const CDentry* get_projected_parent_dn() const
    {
...
public:
    ...
    bool is_dirty_parent()
    {
        return state_test(STATE_DIRTYPARENT);
    }
    bool is_dirty_pool()
    {
        return state_test(STATE_DIRTYPOOL);
    }
...
public:
    ...
    client_t get_loner() const
    {
        return loner_cap;
    }
    client_t get_wanted_loner() const
    {
    ...
    bool is_any_caps()
    {
        return !client_caps.empty();
    }
    bool is_any_nonstale_caps()
    {
        return count_nonstale_caps();
    }

    const compact_map<int32_t,int32_t>& get_mds_caps_wanted() const
    {
        return mds_caps_wanted;
    }
    compact_map<int32_t,int32_t>& get_mds_caps_wanted()
    {
        return mds_caps_wanted;
    }

    const std::map<client_t,Capability*>& get_client_caps() const
    {
        return client_caps;
    }
    ...
    // -- freeze --
    bool is_freezing_inode() const
    {
        return state_test(STATE_FREEZING);
    }
    bool is_frozen_inode() const
    {
        return state_test(STATE_FROZEN);
    }
    bool is_frozen_auth_pin() const
    {
        return state_test(STATE_FROZENAUTHPIN);
    }
