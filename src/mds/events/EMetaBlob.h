...
public:
    ...
    struct fullbit
    {
        ...
        bool is_dirty_parent() const
        {
            return (state & STATE_DIRTYPARENT);
        }
        bool is_dirty_pool() const
        {
            return (state & STATE_DIRTYPOOL);
        }
    ...
    struct remotebit
    {
        ...
        explicit remotebit(bufferlist::iterator &p)
        {
            decode(p);
        }
    ...
    struct nullbit
    {
        ...
        explicit nullbit(bufferlist::iterator &p)
        {
            decode(p);
        }
...
public:
    ...
    public:
        ...
        void mark_dirty_dft()
        {
            state |= STATE_DIRTYDFT;
        }

        const list<ceph::shared_ptr<fullbit> > &get_dfull()   const
        {
            return dfull;
        }
        const list<remotebit>                  &get_dremote() const
        {
            return dremote;
        }
        const list<nullbit>                    &get_dnull()   const
        {
            return dnull;
        }

        void add_dnull(nullbit const &n)
        {
            dnull.push_back(n);
        };
        void add_dfull(ceph::shared_ptr<fullbit> const &p)
        {
            dfull.push_back(p);
        };
        void add_dremote(remotebit const &r)
        {
            dremote.push_back(r);
        };
...
public:
    ...
    entity_name_t get_client_name() const
    {
        return client_name;
    }
