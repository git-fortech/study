...
public:
    ...
    virtual bool IsMergeOperatorSupported() const override
    {
        return false;
    }

    // return false, indicating HashCuckooRep does not support snapshot.
    virtual bool IsSnapshotSupported() const override
    {
        return false;
    }
    ...
    struct CuckooStepBuffer
    {
        ...
        CuckooStep& NextWriteBuffer()
        {
            return steps_[write_index_++];
        }

        inline const CuckooStep& ReadNext()
        {
            return steps_[read_index_++];
        }

        inline bool HasNewWrite()
        {
        ...
        inline bool IsFull()
        {
            return write_index_ >= kCuckooPathMaxSearchSteps;
        }
